...
def test_v4_manifest_appends_without_changing_historical_puzzles():
    ...
    # This reviewed inventory is preserved independently of later additions
    # from the parallel Base test-token branch.
    historical_names = tuple(current["puzzleHashes"])
    assert current["canonicalChecksum"] == hashlib.sha256(
        b"".join(bytes(load_puzzle(name).get_tree_hash()) for name in historical_names)
    ).hexdigest()
    assert all(name in PUZZLE_FILENAMES for name in historical_names)
