...
class TestGenesisValidatorSet:
    PUBKEYS = (b"\x11" * 48, b"\x22" * 48, b"\x33" * 48)

    def test_requires_exactly_two_of_three(self):
        validator_set = require_genesis_validator_set(self.PUBKEYS, 2)
        assert validator_set == ValidatorSet(self.PUBKEYS, 2)
        assert validator_set.policy_hash == make_bridge_policy_hash(self.PUBKEYS, 2)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ((b"\x11" * 48,), 1),
            ((b"\x11" * 48, b"\x22" * 48), 2),
            ((b"\x11" * 48, b"\x22" * 48, b"\x33" * 48), 1),
            ((b"\x11" * 48, b"\x22" * 48, b"\x33" * 48), 3),
        ],
    )
    def test_rejects_non_genesis_quorums(self, pubkeys, threshold):
        with pytest.raises(ValueError, match="exactly three validators at threshold two"):
            require_genesis_validator_set(pubkeys, threshold)

    def test_rejects_duplicate_pubkeys(self):
        with pytest.raises(ValueError, match="duplicate pubkey"):
            require_genesis_validator_set(
                (b"\x11" * 48, b"\x11" * 48, b"\x33" * 48),
                2,
            )
