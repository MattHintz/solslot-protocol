...
RECOVERY_GUARDIANS = tuple(
    "0x" + (bytes([index]) * 20).hex()
    for index in (0x81, 0x82, 0x83)
)
RECOVERY_DRILL_HASHES = tuple(
    bytes32(bytes([index]) * 32)
    for index in (0x91, 0x92, 0x93)
)


def funding_coins(faucet: _FakeFaucet) -> RC23GenesisFundingCoins:
    amounts = (
        1_000_100,
        RC23_POOL_FUNDING_AMOUNT,
        100,
...
def ceremony_plan(
    ...
    return build_rc23_genesis_ceremony_plan(
        ...
        admin_recovery_bls_pubkeys=RECOVERY_KEYS,
        admin_recovery_evm_guardians=RECOVERY_GUARDIANS,
        admin_recovery_revisions=(1, 1, 1),
        admin_recovery_drill_hashes=RECOVERY_DRILL_HASHES,
...
def test_rc23_plan_replaces_nav_registry_with_statutes() -> None:
    faucet = _FakeFaucet()
    ...
    assert payload["adminAuthority"]["compressedPubkeys"] == [
        "0x" + value.hex() for value in ADMIN_KEYS
    ]
    assert [
        item["launcherAmount"]
        for item in payload["adminAuthority"]["identityVaults"]
    ] == [3, 5, 7]
    assert payload["adminAuthority"]["pending"] is False
    assert [
        item["evmGuardian"]
        for item in payload["adminRecoveryKits"]
    ] == list(RECOVERY_GUARDIANS)
    assert [
        item["drillChallengeHash"]
        for item in payload["adminRecoveryKits"]
    ] == ["0x" + value.hex() for value in RECOVERY_DRILL_HASHES]
...
def test_rc23_bundle_keeps_nine_inputs_and_launches_four_authority_vaults() -> None:
    ...
    built = build_rc23_genesis_ceremony_bundle(
        plan=plan,
        faucet=faucet,
        funding_coins=coins,
    )
    ...
    assert plan.admin_authority.launcher_id in spent
    assert all(
        identity.launcher_id in spent
        for identity in plan.admin_authority_v3.identity_vaults
    )
    authority_funding_spend = next(
        spend
        for spend in built.spend_bundle.coin_spends
        if spend.coin.name() == coins.admin_authority.name()
    )
    assert sorted(
        int(coin.amount)
        for coin in compute_additions(authority_funding_spend)
    ) == [1, 3, 5, 7]
...
def test_bridge_batch_has_unique_outputs_and_one_mojo_buffer_fee() -> None:
    ...
    assert RC23_BRIDGE_PARENT_TOTAL == 528
    assert RC23_PROPERTY_REGISTRY_LAUNCHER_AMOUNT == 1
    assert RC23_BRIDGE_BATCH_BUFFER_AMOUNT == 1
    assert RC23_BRIDGE_BATCH_FUNDING_AMOUNT == 530
...
@pytest.mark.parametrize("wrong_amount", (1, 15, 17, 100))
def test_admin_authority_funding_requires_exact_16_mojos(
    wrong_amount: int,
) -> None:
    faucet = _FakeFaucet()
    coins = funding_coins(faucet)
    wrong = replace(
        ...
        admin_authority=Coin(
            coins.admin_authority.parent_coin_info,
            coins.admin_authority.puzzle_hash,
            uint64(wrong_amount),
        ),
    )
    wrong_plan = ceremony_plan(faucet, wrong)
