...
def test_pool_v5_manifest_appends_to_the_unchanged_historical_inventory():
    ...
    historical_names=PUZZLE_FILENAMES[:len(current['puzzleHashes'])]
    assert current['canonicalChecksum']==hashlib.sha256(b''.join(bytes(load_puzzle(name).get_tree_hash()) for name in historical_names)).hexdigest()
    assert current['preservedCanonicalChecksum']==previous['canonicalChecksum']
    assert tuple(current['puzzleHashes'])==historical_names
