#!/usr/bin/env python3
"""Build the deterministic nine-repository RC27.6 source manifest."""
...
RELEASE_ID = "solslot-v2-alpha-rc27.6-20260809"
RELEASE_BRANCH = "release/testnet-alpha-rc27.6-20260809"
...
def build_manifest(
    ...
    *,
    ...
    if release_id != RELEASE_ID:
        raise ValueError("release_id must identify the coordinated RC27.6 alpha release")
    expected_branch = (
...
def verify_release_refs(path: Path, commit: str) -> None:
    ...
    if main_commit != commit or tag_commit != commit:
        raise ValueError(
            f"{path} must have the exact RC27.6 commit on origin/main and {RELEASE_ID}"
        )
...
def build_launch_evidence(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("RC27.6 source manifest is invalid")
    if release_refs_verified is not True:
        raise ValueError(
            "launch evidence requires exact origin/main and RC27.6 tag verification"
        )
    if (
