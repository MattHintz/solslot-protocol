#!/usr/bin/env python3
"""Build the deterministic nine-repository RC27.30 source manifest."""
...
RELEASE_ID = "solslot-v2-alpha-rc27.30-20260821"
RELEASE_BRANCH = "release/testnet-alpha-rc27.30-20260821"
...
def build_manifest(
    ...
    *,
    ...
    if release_id != RELEASE_ID:
        raise ValueError("release_id must identify the coordinated RC27.30 alpha release")
    expected_branch = (
...
def verify_release_refs(path: Path, commit: str) -> None:
    ...
    if main_commit != commit or tag_commit != commit:
        raise ValueError(
            f"{path} must have the exact RC27.30 commit on origin/main and {RELEASE_ID}"
        )
...
def build_launch_evidence(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("RC27.30 source manifest is invalid")
    if release_refs_verified is not True:
        raise ValueError(
            "launch evidence requires exact origin/main and RC27.30 tag verification"
        )
    if (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.evidence_output:
        ...
        if not isinstance(puzzle_inventory, Mapping):
            raise ValueError("RC27 puzzle inventory must be an object")
